...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, bitrate_dim, name):
        self.sess = sess
        ...
        self.inputs, self.out = self.create_actor_network(name)

        # Get all network parameters
        self.network_params = \
            tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope=name)
    ...
    def create_actor_network(self, name: str):
...
class CriticNetwork(object):
    ...
    """
    def __init__(self, sess, state_dim, learning_rate, bitrate_dim, name):
        self.sess = sess
        self.s_dim = state_dim
        self.lr_rate = learning_rate
        self.bitrate_dim = bitrate_dim
        ...
        self.inputs, self.out = self.create_critic_network(name)
    ...
    def create_critic_network(self, name: str):
        with tf.variable_scope(name):
            inputs = tflearn.input_data(shape=[None, self.s_dim[0], self.s_dim[1]])
...
def compute_gradients(s_batch, a_batch, r_batch, terminal, actor, critic, entropy_weight):
    ...
    # np.random.shuffle(s_batch)

    v_batch = critic.predict(s_batch)
    R_batch = np.zeros(r_batch.shape)
    # np.random.shuffle(s_batch)
    # tt = critic.predict(s_batch)
