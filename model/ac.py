...
class ActorCritic:
    ...
    # 模型更新
    from sac_pic import Memory
    def update(self, memory: Memory, epoch: int):
        if memory.memory_counter < memory.capacity:
            return
        # 训练集
        if epoch % 100 != 0:
            return
        b_M = memory.sample(1024)
        ...
        actions = torch.tensor(b_a, dtype=torch.int64).view(-1, 1)
        rewards = torch.FloatTensor(b_r).view(-1, 1)
        next_states = torch.FloatTensor(b_s_)
        # dones = torch
