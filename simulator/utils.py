...
from simulator.constants import (HD_REWARD, M_IN_K, REBUF_PENALTY,
...
def linear_reward(current_bitrate: int, last_bitrate: int, rebuffer: float):
    ...
    # print(str(current_bitrate / M_IN_K) + " " + str(REBUF_PENALTY * rebuffer) + " " + str(SMOOTH_PENALTY * np.abs(current_bitrate - last_bitrate) / M_IN_K))
    return reward
